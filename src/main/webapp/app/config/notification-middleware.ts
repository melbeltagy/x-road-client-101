...
import { FieldErrorVM, isProblemWithMessage } from 'app/shared/error/problem-details';
import { getMessageFromHeaders } from 'app/shared/error/headers';
...
const getFieldErrorsToasts = (fieldErrors: FieldErrorVM[]): ToastMessage[] =>
  fieldErrors.map(fieldError => {
    ...
    const fieldName = translate(`xRoadExampleRestapiClientApp.${fieldError.objectName}.${convertedField}`);
