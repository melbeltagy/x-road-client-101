...
describe("security-server.service", () => {
  ...
  describe("fetchRegisteredClients", () => {
    ...
    it("should return list of clients", async () => {
      const clients: SubsystemId[] = [createMockSubsystem({ subsystemCode: "Client1" }), createMockSubsystem({ subsystemCode: "Client2" })];
      vi.mocked(axios.get).mockResolvedValueOnce({ data: clients });
    ...
    it("should propagate API errors", async () => {
      ...
      await expect(fetchRegisteredClients("https://ss.example.com")).rejects.toThrow("Network Error");
    });
  });
  ...
  describe("fetchServices", () => {
    ...
    it("should propagate API errors", async () => {
      ...
      await expect(fetchServices("https://ss.example.com", clientSubsystem, serviceSubsystem)).rejects.toThrow("Service unavailable");
    });
  });
});
