...
const fullClient: SubsystemId = {
  instanceId: 'TEST', memberClass: 'GOV', memberCode: '1234567-8', subsystemCode: 'C',
};
const fullService: SubsystemId = {
  instanceId: 'TEST', memberClass: 'GOV', memberCode: '9876543-2', subsystemCode: 'S',
};
...
describe('useServicesLoader', () => {
  ...
  describe('input-gating (does not fetch until inputs are valid + complete)', () => {
    it('does not fetch with empty URL', async () => {
      ...
      useServicesLoader(() => url.value, () => client.value, () => service.value);
  ...
  describe('happy path', () => {
    it('fetches services when all inputs become valid', async () => {
      ...
      const loader = useServicesLoader(
        () => url.value, () => client.value, () => service.value
      );
      ...
      expect(fetchServices).toHaveBeenCalledWith(
        'https://ss.example.com', fullClient, fullService
      );
  ...
  describe('debounce', () => {
    it('coalesces rapid input changes into a single fetch', async () => {
      ...
      expect(fetchServices).toHaveBeenLastCalledWith(
        'https://ss3.example.com', fullClient, fullService
      );
    });
