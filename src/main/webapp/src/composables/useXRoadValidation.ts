...
export function useXRoadValidation() {
  ...
  function validateSubsystem(subsystem: SubsystemId, prefix: "client" | "service", newErrors: Record<string, string>): void {
    // Instance ID
  ...
  function validateRequest(request: { method: string; path: string }, newErrors: Record<string, string>): void {
    if (!request.method) {
