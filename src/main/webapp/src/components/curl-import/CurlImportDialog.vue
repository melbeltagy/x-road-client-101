...
const canImport = computed(() => !!parseResult.value && parseResult.value.request !== null && !parseResult.value.error);
  ...
  <v-dialog :model-value="modelValue" max-width="900" persistent @update:model-value="emit('update:modelValue', $event)">
    <v-card>
      ...
      <v-card-text>
        ...
        <TextareaWithErrorHighlight v-model="curlText" :error-span="errorSpan" :placeholder="PLACEHOLDER_CURL" autofocus />

        <v-alert v-if="error" type="error" variant="tonal" density="compact" class="mt-3">
          {{ error }}
        </v-alert>

        <v-alert v-for="(w, idx) in warnings" :key="idx" type="warning" variant="tonal" density="compact" class="mt-3">
          {{ w }}
        </v-alert>
      </v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" variant="flat" :disabled="!canImport" @click="handleImport">
