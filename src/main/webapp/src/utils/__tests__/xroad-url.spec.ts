...
import { formatXRoadClient, buildServicePath, buildServiceUrl, isValidHttpUrl } from "../xroad-url";
...
describe("xroad-url utilities", () => {
  ...
  describe("isValidHttpUrl", () => {
    it.each(["http://example.com", "https://example.com", "http://localhost:8080", "https://ss.example.com:8443/path?q=1"])(
      "accepts %s",
      (url) => {
    ...
    it.each(["ftp://example.com", "file:///etc/passwd", "data:text/plain,foo"])("rejects non-HTTP scheme (%s)", (url) => {
      expect(isValidHttpUrl(url)).toBe(false);
    });
